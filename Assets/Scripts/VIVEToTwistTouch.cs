...
{
    public class VIVEToTwistTouch : UnityPublisher<MessageTypes.Geometry.Twist>
    {
        ...
        private void Update()
        {
            ...
            //     print("SnapLeft");
            // }
            // print("Aixs" + TouchpadAction.GetAxis(handType));
            direction = TouchpadAction.GetAxis(handType);
            // ordertext.text = "当前指令值是" + direction;
        ...
        {
            message.linear.x = linerspeed * direction.y;
            message.linear.y = 0;
            message.linear.z = 0;
            message.angular.z = angularconvert * direction.x;
            message.angular.x = 0;
            message.angular.y = 0;
            Publish(message);
        }
